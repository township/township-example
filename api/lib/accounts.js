...
module.exports = function (db, config) {
  ...
  assert.equal(typeof config.email, 'object', 'config.email object required')
  ...
  */
  ...
  accounts.getToken = function getToken (req) {
    ...
    if (authHeader && authHeader.indexOf('Bearer') > -1) {
      ...
      token = cookie(req.headers.cookie).get('access_token')
    }
    return token
  }
  ...
  */
  ...
  accounts.setCookie = function setCookie (res, options) {
    ...
    res.setHeader('Set-Cookie', 'access_token=' + token + '; ' + 'path=/; domain=' + hostname + '; HttpOnly')
  }
